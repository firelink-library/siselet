...
import dopagemSilicio from '../../../assets/digi/transistor/dopagem_silicio.png';
import juncaoPn from '../../../assets/digi/transistor/juncao_pn.png';
import polarizacaoDiodo from '../../../assets/digi/transistor/polarizacao_diodo.gif';
import polarizacaoReversa from '../../../assets/digi/transistor/diodo_polarizacao_reversa.gif';
import diodo from '../../../assets/digi/transistor/diodo.jpg';
import transistorBjt from '../../../assets/digi/transistor/transistor_bjt.png';
import transistorNpn from '../../../assets/digi/transistor/transistor_npn.jpg';
import fluxoBjt from '../../../assets/digi/transistor/fluxo_bjt.jpg';
import fluxoNpn from '../../../assets/digi/transistor/fluxo_npn.jpg';
import mosfet from '../../../assets/digi/transistor/mosfet.jpg';
import mosfetInterno from '../../../assets/digi/transistor/mosfet_interno.jpg';
import capacitor from '../../../assets/digi/transistor/capacitor.webp';
import capacitorCarregando from '../../../assets/digi/transistor/capacitor_carregando.png';
import mosCapacitor from '../../../assets/digi/transistor/mos_capacitor.png';
import mosfetN from '../../../assets/digi/transistor/mosfet_n.webp';
...
<p><center>O processo de dopagem do silício. Em a), temos o silício
...
Veritasium em que ele conta a história e a importância do *LED azul*.

:::note[Autoestudo obrigatório]

O trecho relevante começa em 3:49 e vai até 9:20, mas o vídeo é fantástico. Se
tiver meia horinha sobrando aí, assista ele inteiro!
...
<p><center>A junção pn ocorre quando um semicondutor do tipo n e um
...
:::note[Conceito Chave: Zona de Depleção]

A **zona de depleção** (ou região de depleção) é uma camada isolante que se forma na junção entre os materiais P e N. Nela, os portadores de carga se recombinam e "somem", criando uma barreira que impede a passagem de corrente a menos que uma tensão externa seja aplicada para vencê-la.
...
<center>
  <Image
    src={polarizacaoDiodo}
    alt="Polarização direta"
    style={{ height: 'auto' }}
  />
</center>
<p><center>Polarização direta de uma junção PN. A região de depleção
...
<center>
  <Image
    src={polarizacaoReversa}
    alt="Polarização reversa"
    style={{ height: 'auto' }}
  />
</center>
<p><center>Polarização reversa de junção pn. A região de depleção
aumenta.</center></p>
...
<p><center>O diodo é um componente que permite a passagem de
...
<p><center>O transistor de junção bipolar npn tem três zonas de
material semicondutor, sendo uma tipo p sanduichada entre duas regiões tipo
n.</center></p>

A resposta? Adicionar três zonas polarizadas, o que faz com que existam duas
zonas de depleção. Aqui, vale a pena também falar sobre a nomenclatura das três
...
* O coletor (C) é onde tipicamente conecta-se uma fonte de tensão de alta
  ...
  coletor para;
* O emissor (E), que tipicamente é conectado ao restante do circuito (assumindo
  o uso de um transistor como amplificador).

:::tip[Analogia da Torneira]
Para facilitar, imagine o BJT como uma torneira de jardim:
* O **Coletor** é a mangueira onde a água (corrente) está esperando para sair.
* O **Emissor** é o bocal por onde a água sai.
* A **Base** é o registro. Se você girar só um pouquinho o registro (pequena corrente na base), sai um fluxo muito maior de água pelo bocal (corrente amplificada no coletor). Se fechar o registro, a água para de correr (corte).
:::

Para entender como o transistor BJT npn funciona é conveniente representar as
...
<p><center>Transistor npn BJT com suas regiões bipolares como
diodos.</center></p>
...
<p><center>Fluxo de elétrons em um transistor npn em região de
...
**chave** eletronicamente controlada.

No modo amplificador, a relação entre a corrente que entra na base ($I_B$) e a corrente que flui pelo coletor ($I_C$) é definida pelo ganho do transistor ($\beta$ ou $h_{FE}$):

$$I_C = \beta \cdot I_B$$

Isso significa que uma corrente minúscula na base pode controlar uma corrente muito maior no coletor!

| Estado | Chave | Corrente de Base ($I_B$) | Corrente de Coletor ($I_C$) | Tensão Coletor-Emissor ($V_{CE}$) |
| :--- | :--- | :--- | :--- | :--- |
| **Corte** | Aberta (OFF) | Zero ou negativa | Zero | Máxima (igual a $V_{CC}$) |
| **Saturação** | Fechada (ON) | Alta o suficiente | Máxima (limitada pela carga) | Mínima ($\approx 0,2V$) |
...
<p><center>Vista de seção de um transistor BJT npn.</center></p>
...
efeito de campo de metal-óxido-semicondutor**, ou **MOSFET**.
...
<p><center>Um MOSFET *tipo n*.</center></p>

Enquanto o BJT tinha *Base*, *Coletor* e *Emissor*, o MOSFET tem *Gate*,
*Drain* e *Source*. Há uma equivalência funcional entre essas portas:
...
Do ponto de vista de dopagem, o MOSFET é como se aplicássemos seletivamente
...
<p><center>O MOSFET por dentro. Há uma região principal, da qual o
...
Se aplicarmos uma tensão no sentido de DS (*Drain - Source*), assim como no
...
<p><center>Um capacitor eletrolítico tipicamente é um enrolamento de
...
<center>
  <Image
    src={capacitorCarregando}
    alt="Circuito c/ capacitor"
    style={{ height: 'auto' }}
  />
</center>
<p><center>Um capacitor quando alimentado por uma fonte de tensão.</center></p>
...
<p><center>O capacitor MOS (metal-óxido-semicondutor) é uma
configuração possível de um capacitor. Substitui-se uma das placas de metal por
material semicondutor.</center></p>

**Cria-se um canal**

Vamos considerar o caso do MOSFET *tipo n* e novamente aplicar uma tensão no
...
*tipo p* faz com que os buracos do substrato sejam repelidos e crie-se uma
nova zona de depleção e, dentro dela, um canal que efetivamente conecta as
regiões de *source* e *drain*, permitindo a movimentação dos elétrons. Assim, o
MOSFET conduz.
...
<p><center>Um MOSFET tipo n, quando recebe tensão positiva em seu
*gate*, gera um canal por onde é possível passar corrente entre o *drain* e o
*source*</center></p>

:::tip[Exercício 4.03]

É. Chegou a hora. Eu mostrei o MOSFET *tipo n* para vocês. Qual é a diferença
na prática de um *tipo n* para um *tipo p*? Muda alguma coisa na maneira como
devo usá-lo?
...
Beleza, mas por que usar um componente com um comportamento tão mais complexo
